...
namespace Enwbvs\Enweby\SettingsFramework;
...
class WordPressSettingsFramework {
	...
	public function generate_image_radio_field( $args ) {
		...
		foreach ( $args['choices'] as $value => $choice ) {
			$field_id = sprintf( '%s_%s', $args['id'], $value );
			$checked  = $value == $args['value'] ? 'checked="checked"' : ''; // phpcs:ignore
	...
	public function generate_group_row_template( $args, $blank = false, $row = 0 ) {
		...
		if ( $args['subfields'] ) {
			$row_class = ( $row % 2 == 0 ) ? 'alternate' : ''; // phpcs:ignore
	...
	public function generate_radio_field( $args ) {
		...
		foreach ( $args['choices'] as $value => $text ) {
			...
			echo sprintf( '<label><input type="radio" name="%s" id="%s" value="%s" class="%s" %s> %s</label><br />', esc_attr( $args['name'] ), esc_attr( $field_id ), esc_attr( $value ), esc_attr( $args['class'] ), esc_attr( $checked ), esc_attr( $text ) );
		}
			...
			foreach ( $this->tabs as $tab_data ) {
				...
				?>
					<li class="wpsf-nav__item <?php echo esc_attr( $active ); ?>">
						<a class="wpsf-nav__item-link <?php echo esc_attr( $tab_data['class'] ); ?>" href="#tab-<?php echo esc_attr( $tab_data['id'] ); ?>"><?php echo esc_attr( $tab_data['title'] ); ?></a>
					</li>
					<?php
					$i ++;
			}
